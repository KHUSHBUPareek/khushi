...
features[:, 0] = labelencoder.fit_transform(features[:, 0])#It can also be used to transform non-numerical labels (as long as they are hashable and comparable) to numerical labels.

#kk onehot
#kk The input to this transformer should be a matrix of integers, denoting the values taken on by categorical (discrete) features.
#kk The output will be a sparse matrix where each column corresponds to one possible value of one feature.
#kk It is assumed that input features take on values in the range [0, n_values).
from sklearn.preprocessing import OneHotEncoder
onehotencoder = OneHotEncoder(categorical_features = [0])#kk-->categorical_features = [0]-->means on 0th coloumn of features
